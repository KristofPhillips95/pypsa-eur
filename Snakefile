...
rule add_electricity:
    input:
        ...
        **{
        ...
        nuts3_shapes="resources/" + RDIR + "nuts3_shapes.geojson",
    output:
        "networks/" + RDIR + "elec_{regions}.nc",
    log:
        "logs/" + RDIR + "add_electricity_{regions}.log",
    benchmark:
        "benchmarks/" + RDIR + "add_electricity_{regions}"
    threads: 1
    resources:
        mem_mb=5000,
    script:
...
rule simplify_network:
    input:
        network="networks/" + RDIR + "elec_{regions}.nc",
        tech_costs=COSTS,
        regions_onshore="resources/" + RDIR + "regions_onshore.geojson",
        regions_offshore="resources/" + RDIR + "regions_offshore.geojson",
    output:
        ...
        busmap="resources/" + RDIR + "busmap_elec_s{simpl}_{regions}.csv",
        connection_costs="resources/" + RDIR + "connection_costs_s{simpl}_{regions}.csv",
    log:
        "logs/" + RDIR + "simplify_network/elec_s{simpl}_{regions}.log",
    benchmark:
        "benchmarks/" + RDIR + "simplify_network/elec_s{simpl}_{regions}"
    threads: 1
    resources:
        mem_mb=4000,
    script:
...
rule cluster_network:
    input:
        ...
        tech_costs=COSTS,
    output:
        network="networks/" + RDIR + "elec_s{simpl}_{clusters}_{regions}.nc",
        regions_onshore="resources/"
        + RDIR
        + "regions_onshore_elec_s{simpl}_{clusters}_{regions}.geojson",
        regions_offshore="resources/"
        + RDIR
        + "regions_offshore_elec_s{simpl}_{clusters}_{regions}.geojson",
        busmap="resources/" + RDIR + "busmap_elec_s{simpl}_{clusters}_{regions}.csv",
        linemap="resources/" + RDIR + "linemap_elec_s{simpl}_{clusters}_{regions}.csv",
    log:
        "logs/" + RDIR + "cluster_network/elec_s{simpl}_{clusters}_{regions}.log",
    benchmark:
        "benchmarks/" + RDIR + "cluster_network/elec_s{simpl}_{clusters}_{regions}"
    threads: 1
    resources:
        mem_mb=6000,
    script:
...
rule add_extra_components:
    input:
        ...
        tech_costs=COSTS,
    output:
        "networks/" + RDIR + "elec_s{simpl}_{clusters}_{regions}_ec.nc",
    log:
        "logs/" + RDIR + "add_extra_components/elec_s{simpl}_{clusters}_{regions}.log",
    benchmark:
        "benchmarks/" + RDIR + "add_extra_components/elec_s{simpl}_{clusters}_{regions}_ec"
    threads: 1
    resources:
        mem_mb=3000,
    script:
...
rule prepare_network:
    ...
    log:
        "logs/" + RDIR + "prepare_network/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}.log",
    benchmark:
        (
            "benchmarks/"
            + RDIR
            + "prepare_network/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}"
        )
    threads: 1
    resources:
        mem_mb=4000,
...
rule solve_network:
    ...
    log:
        solver=normpath(
            "logs/"
            + RDIR
            + "solve_network/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_solver.log"
        ),
        python="logs/"
        + RDIR
        + "solve_network/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_python.log",
        memory="logs/"
        + RDIR
...
rule plot_network:
    input:
        ...
        + RDIR
        + "elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}.nc",
        tech_costs=COSTS,
    output:
        only_map="results/plots/"
        + RDIR
        + "elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_{attr}.{ext}",
        ext="results/plots/"
        + RDIR
        + "elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_{attr}_ext.{ext}",
    log:
        "logs/"
        + RDIR
        + "plot_network/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_{attr}_{ext}.log",
    script:
...
rule make_summary:
    ...
    output:
        directory(
            "results/summaries/"
            + RDIR
            + "elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_{country}"
        ),
    log:
        "logs/"
        + RDIR
        + "make_summary/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_{country}.log",
    resources:
        mem_mb=500,
    script:
...
rule plot_summary:
    ...
    output:
        "results/plots/"
        + RDIR
        + "summary_{summary}_elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_{country}.{ext}",
    log:
        "logs/"
        + RDIR
        + "plot_summary/{summary}_elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_{country}_{ext}.log",
    resources:
        mem_mb=500,
    script:
