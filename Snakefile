...
#Or better yet, include it in opts
...
if config['enable'].get('build_cutout', False):
    rule build_cutout:
        input:
            regions_onshore="resources/regions_onshore.geojson",
            regions_offshore="resources/regions_offshore.geojson",
            regions_renewable="resources/regions_renewable.geojson"
...
rule add_electricity:
    ...
    output: "networks/elec_{regions}.nc"
    log: "logs/add_electricity_{regions}.log"
    benchmark: "benchmarks/add_electricity_{regions}"
    threads: 1
    resources: mem_mb=5000
...
rule simplify_network:
    input:
        network='networks/elec_{regions}.nc',
        tech_costs=COSTS,
        regions_onshore="resources/regions_onshore.geojson",
        regions_offshore="resources/regions_offshore.geojson"
    output:
        ...
        busmap='resources/busmap_elec_s{simpl}_{regions}.csv',
        connection_costs='resources/connection_costs_s{simpl}_{regions}.csv'
    log: "logs/simplify_network/elec_s{simpl}_{regions}.log"
    benchmark: "benchmarks/simplify_network/elec_s{simpl}_{regions}"
    threads: 1
...
rule cluster_network:
    input:
        ...
        tech_costs=COSTS
    output:
        network='networks/elec_s{simpl}_{clusters}_{regions}.nc',
        regions_onshore="resources/regions_onshore_elec_s{simpl}_{clusters}_{regions}.geojson",
        regions_offshore="resources/regions_offshore_elec_s{simpl}_{clusters}_{regions}.geojson",
        busmap="resources/busmap_elec_s{simpl}_{clusters}_{regions}.csv",
        linemap="resources/linemap_elec_s{simpl}_{clusters}_{regions}.csv"
    log: "logs/cluster_network/elec_s{simpl}_{clusters}_{regions}.log"
    benchmark: "benchmarks/cluster_network/elec_s{simpl}_{clusters}_{regions}"
    threads: 1
    resources: mem_mb=6000
    script: "scripts/cluster_network_new.py"
...
rule add_extra_components:
    input:
        network='networks/elec_s{simpl}_{clusters}_{regions}.nc',
        tech_costs=COSTS,
    output: 'networks/elec_s{simpl}_{clusters}_{regions}_ec.nc'
    log: "logs/add_extra_components/elec_s{simpl}_{clusters}_{regions}.log"
    benchmark: "benchmarks/add_extra_components/elec_s{simpl}_{clusters}_{regions}_ec"
    threads: 1
...
rule prepare_network:
    input: 'networks/elec_s{simpl}_{clusters}_{regions}_ec.nc', tech_costs=COSTS
    output: 'networks/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}.nc'
    log: "logs/prepare_network/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}.log"
    benchmark: "benchmarks/prepare_network/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}"
    threads: 1
    resources: mem_mb=4000
...
rule solve_network:
    input: "networks/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}.nc"
    output: "results/networks/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}.nc"
    log:
        solver=normpath("logs/solve_network/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_solver.log"),
        python="logs/solve_network/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_python.log",
        memory="logs/solve_network/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_memory.log"
    benchmark: "benchmarks/solve_network/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}"
    threads: 4
...
rule plot_network:
    input:
        network="results/networks/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}.nc",
        tech_costs=COSTS
    output:
        only_map="results/plots/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_{attr}.{ext}",
        ext="results/plots/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_{attr}_ext.{ext}"
    log: "logs/plot_network/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_{attr}_{ext}.log"
...
def input_make_summary(w):
    ...
    if w.ll.endswith("all"):
        ...
        ll = w.ll
    return ([COSTS] +
            expand("results/networks/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}.nc",
                   ll=ll, regions = w.regions,
...
rule make_summary:
    ...
    output: directory("results/summaries/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_{country}")
    log: "logs/make_summary/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_{country}.log",
    script: "scripts/make_summary.py"


rule plot_summary:
    input: "results/summaries/elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_{country}"
    output: "results/plots/summary_{summary}_elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_{country}.{ext}"
    log: "logs/plot_summary/{summary}_elec_s{simpl}_{clusters}_{regions}_ec_l{ll}_{opts}_{country}_{ext}.log"
