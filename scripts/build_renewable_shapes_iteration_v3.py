...
def get_loc_weight(regions : str):
    if "loc" in regions:
        loc_weight = float(regions.partition("loc")[2])
    else:
        loc_weight = 0
    return loc_weight
...
#    plot_selection_attr = snakemake.config["renewable_zones"]["plot_selection_attr"]
    loc_weight = get_loc_weight(snakemake.wildcards.regions)
    ...
    if snakemake.wildcards.regions == "c":
        ...
        nb_regions = int(snakemake.wildcards.regions.split("-")[1])
