...
if __name__ == "__main__":
    ...
    if snakemake.wildcards.regions == "c":
        ...
        path_tosave_to = os.path.join(os.path.dirname(os.path.dirname(snakemake.output.renewable_shapes)), "results" , f"gdfs_{loc_weight}_{parallel}")
        circ_min = float("inf")
        seed_min = 0
        for seed in gdfs:
            gdfs[seed]["dist_tot"] = dist_per_seed[seed]
            gdfs[seed]["circumference"] = circumference[seed]
            gdfs[seed]["total_time"] = tock-tick
            ...
            # if not(os.path.exists(f"../results/gdfs")):
            #     os.makedirs(f"../results/gdfs")
            rsm.save_to_geojson(gdfs[seed],os.path.join(path_tosave_to,f"ren_clust_{snakemake.wildcards.regions}_{gran}_{seed}"))

        rsm.plot_regions_scatter_gran_and_save(lat_all_gran, lon_all_gran, model,labels_dict[seed_min], centroids_dict[seed_min],seed,snakemake.output,gran)
